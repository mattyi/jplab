...
public class EdgePainter extends JavaFxPainter<Edge> {
  ...
  public void paint(
    ...
    double endAX = edge.length() * Math.cos(theta + (Math.PI / 2)) / 2;
    double endAY = edge.length() * Math.sin(theta + (Math.PI / 2)) / 2;
    ...
    for (int i = 1; i <= edge.innerLineCount(); i++) {
      start.x(i * (endBX - endAX) / (edge.innerLineCount() + 1));
      start.y(i * (endBY - endAY) / (edge.innerLineCount() + 1));
