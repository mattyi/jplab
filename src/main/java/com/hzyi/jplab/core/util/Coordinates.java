...
public class Coordinates {
   ...
   *
   */
  public static Coordinate transform(Coordinate coordinate, CoordinateSystem from, CoordinateSystem to) {
    double cnx = coordinate.x() * from.ux().x() + coordinate.y() * from.uy().x() + from.origin().x() - to.origin().x();
    double cny = coordinate.y() * from.uy().y() + coordinate.x() * from.ux().y() + from.origin().y() - to.origin().y();
    double ctx = (cnx * to.uy().y() - cny * to.uy().x()) / (to.ux().x() * to.uy().y() - to.ux().y() * to.uy().x());
    double cty = (cnx * to.ux().y() - cny * to.ux().x()) / (to.uy().x() * to.ux().y() - to.uy().y() * to.ux().x());
    return new Coordinate(ctx, cty);
  }

  public static void transform(Coordinate coordinateFrom, Coordinate coordinateTo, CoordinateSystem from, CoordinateSystem to) {
